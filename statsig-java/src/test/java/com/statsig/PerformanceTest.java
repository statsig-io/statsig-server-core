...
public class PerformanceTest {
  ...
  public void test() throws ExecutionException, InterruptedException {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      StatsigUser user =
          new StatsigUser.Builder().setUserID(currUser).setEmail("weihao@statsig.com").build();
      try {
        result =
            statsig.getClientInitializeResponse(
                user, new ClientInitResponseOptions(HashAlgo.NONE, null));
      } catch (Exception e) {
        System.err.println(
            "Error initializing client response for user " + currUser + ": " + e.getMessage());
      }
    }
