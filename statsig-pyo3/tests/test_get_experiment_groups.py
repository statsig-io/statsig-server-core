...
def test_get_experiment_groups_returns_typed_result(statsig_setup):
    statsig = statsig_setup
    result = statsig.get_experiment_groups("test_experiment_no_targeting")

    assert isinstance(result, ExperimentGroupsResult)
    assert all(isinstance(group, ExperimentGroup) for group in result.groups)
