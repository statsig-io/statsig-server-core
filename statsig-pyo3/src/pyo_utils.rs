...
pub fn call_py_function_with_no_args(func_name: &str, func_opt: &Option<PyObject>) {
    if func_opt.is_none() {
        println!("{}: No function to call for {}", TAG, func_name);
        return;
    }
    ...
    Python::with_gil(|py| {
        let func = match func_opt.as_ref() {
            Some(f) => f,
            None => {
                println!("{}: Function is None for {}", TAG, func_name);
                return;
            }
        };
...
pub fn call_py_function_with_two_args(
    func_name: &str,
