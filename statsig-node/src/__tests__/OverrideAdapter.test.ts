...
import { OverrideAdapterType, Statsig, StatsigOptions, StatsigUser } from "../../build";
...
describe('Override Adapter Usage', () => {
    ...
    beforeAll(async () => {
        ...
        const dcs = fs.readFileSync(
            path.join(__dirname, '../../../statsig-lib/tests/data/eval_proj_dcs.json'),
            'utf8',
        );
    
        scrapi.mock('/v2/download_config_specs', dcs, { status: 200, method: 'GET' });
        scrapi.mock('/v1/log_event', '{"success": true}', { status: 202, method: 'POST' });
    ...
    const testCases = [
        ...
        },
        {
            ...
            optionsModifier: (opts: StatsigOptions) => ({ ...opts, overrideAdapterConfig: [{
        ...
        describe(title, () => {
            ...
            const overrideTests = [
                ...
                },
                {
                    ...
                    action: () => statsig.overrideDynamicConfig('operating_system_config', { foo: 'bar' }),
                    getter: () => statsig.getDynamicConfig(user, 'operating_system_config'),
                    expectedValue: { foo: 'bar' },
                },
                {
                    name: 'should override experiments',
                    action: () => statsig.overrideExperiment('test_exp_random_id', { foo: 'bar' }),
                    getter: () => statsig.getExperiment(user, 'test_exp_random_id'),
                    expectedValue: { foo: 'bar' },
                },
                {
                    name: 'should override experiments by group name',
                    action: () => statsig.overrideExperimentByGroupName('test_exp_random_id', 'Control'),
                    getter: () => statsig.getExperiment(user, 'test_exp_random_id'),
