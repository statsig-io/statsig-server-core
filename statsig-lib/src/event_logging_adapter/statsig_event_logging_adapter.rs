...
impl EventLoggingAdapter for StatsigEventLoggingAdapter {
        ...
        match self.context.write() {
            Ok(mut context) => {
                ...
                *context = Some(Arc::new(AdapterContext {
                    log_event_url: url,
                    network: NetworkClient::new(Some(
                        StatsigMetadata::get_constant_request_headers(sdk_key),
                    )),
                }));
            }
    ...
    async fn log_events(
        ...
        let response = context.network.post(RequestArgs {
...
async fn test_event_logging() {
    ...
    adapter.bind(&sdk_key, &StatsigOptions::new());
